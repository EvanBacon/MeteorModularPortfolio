SlidesController = MeteorisController.extend({
  ...
  index: function() {
    ...
    return this.render('slidesIndex', {
      data: {
  ...
  _getDoc: function(t) {
    return {
      ...
      isSlide: t.find('#isSlide').checked,
    };
  },
  ...
  insert: function(t) {
    if (this._post) {
      ...
      Slides.insert(doc, function(err, _id) {
        if (err) {
          MeteorisFlash.set('danger', err.message);
          throw new Meteor.Error(err);
        }
        MeteorisFlash.set('success', "Success Inserting Slides");
        Router.go('slidesView', {_id: _id});
      });
    }
    return this.render('slidesInsert', {});
  },
  /* event updating data */
  update: function(t) {
    var _id = this.getId();
    ...
    if (this._post) {
      ...
      Slides.update(_id, {$set: doc}, function(err) {
        if (err) {
          MeteorisFlash.set('danger', err.message);
          throw new Meteor.Error(err);
        }
        MeteorisFlash.set('success', "Success Updating Slides");
      });
      Router.go('slidesView', {_id: _id});
    }
    return this.render('slidesUpdate', {
      data: {
        model: model,
      }
    });
  ...
  remove: function(_id) {
    Slides.remove(_id, function(err) {
      if (err) {
        MeteorisFlash.set('danger', err.message);
        throw new Meteor.Error(err);
      }
      MeteorisFlash.set('success', "Success Removing Slides");
    });
  },
