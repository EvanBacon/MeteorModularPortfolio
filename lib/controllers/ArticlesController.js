ArticlesController = MeteorisController.extend({
    /* get subscribtion from server with parameter criteria, and sort/limit */
    subscriptions: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();

        this.subs.subscribe('articles', {});
this.subs.subscribe('articles', {});
this.subs.subscribe('articles', {});

        this.subscription = this.subs.subscribe('articles', this.getCriteria(), sort);
    },
...
{date: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Articles.find(this.getCriteria(), sort);
...
date: t.find('#date').value? new Date(t.find('#date').value):null,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Articles.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Articles");
                Router.go('articlesView', {_id: _id});
            });
        }
        return this.render('articlesInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Articles.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Articles");
            });
            Router.go('articlesView', {_id: _id});
        }
        return this.render('articlesUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Articles.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Articles");
        });
    },
    _loadModel: function(_id) {
        return Articles.findOne(_id);
    },
});
